...
from tensorpack import dataflow, LMDBData
import lmdb
...
def main():
    ...
    # env = lmdb.open(lmdb_file_path, subdir=False, readonly=True, lock=False, readahead=True)
    # data_b = env.begin()
    # size = data_b.stat()['entries']

    # keys = data_b.get(b'__keys__')


    df = dataflow.LMDBSerializer.load(lmdb_file_path, shuffle=False)

    # df = LMDBData(lmdb_file_path, shuffle=False)
