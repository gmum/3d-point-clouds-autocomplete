...
def process(shape_dir):
    ...
    #         comp = completeness(existing_pcs[i], sample_pts)
    #         gen_comp += comp
    ...
    #     gen_pcs_tensors = torch.stack(gen_pcs_tensors, dim=0)
    #
    #     existing_pc_tensor = torch.tensor(existing_pcs[i]).transpose(1, 0)
    #
    #     existing_pc_tensor = existing_pc_tensor.unsqueeze(0).repeat((gen_pcs_tensors.size(0), 1, 1))
    #
    #     hausdorff = directed_hausdorff(existing_pc_tensor, gen_pcs_tensors, reduce_mean=True).item()
